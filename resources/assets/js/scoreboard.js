...
$(document).ready(function() {
	$('[data-control="chart-pie"]').each(function(index, el) {
		var values = [],
			colors = [];
		...
		$(this).find('li').each(function(index, el) {
			...
			$(this).prepend('<i style="background: '+$(this).attr('data-color')+'"></i>');
		});

		chart.text(values.join());
		chart.peity('donut', { "fill": colors, "innerRadius": 14, "radius": 18, "height": 48, "width": 48 });
	});

	$('[data-control="chart-bar"]').each(function(index, el) {
		var values = [],
			colors = [];
			chart = $(this).find('.chart');
		...
		chart.text(values.join());
		chart.peity('bar', { "fill": colors, "height": 64, "width": 64 });
	});
});