...
(function($) {
	$.fn.connect = function(param) {
		...
		_canvas = $('<canvas/>').addClass('canva-lines')
				...
				*/
				...
				if (option.left_node != '' && typeof option.left_node !== 'undefined' && option.right_node != '' && typeof option.right_node !== 'undefined' && $(option.left_node).length > 0 && $(option.right_node).length > 0) {
					...
					$(option.right_node).each(function(index, value) {
						...
						_offset_top = _me.offset().top;
						_offset_left = _me.offset().left;

						//Get Left point and Right Point
						_left.x = (_left_node.offset().left - _offset_left) + _left_node.outerWidth();
						_left.y = (_left_node.offset().top - _offset_top) + (_left_node.outerHeight() / 2);
						_right.x = _right_node.offset().left - _offset_left;
						_right.y = (_right_node.offset().top - _offset_top) + (_right_node.outerHeight() / 2);
