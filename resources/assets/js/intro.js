...
(function (root, factory) {
  ...
  function IntroJs(obj) {
    ...
    $(obj).append(topMask, bottomMask, leftMask, rightMask);
...
function _getElementAttrs (element) {
	return {
...
function _positionMask (el) {
	if(!el.element)
		return;

	var element = $(el.element),
	margin = 5,
	attrs = _getElementAttrs(element),
	top = attrs.top,
	left = attrs.left,
	width = attrs.width,
	height = attrs.height;
  ...
  */
  ...
  function _setHelperLayerPosition(helperLayer) {
    if (helperLayer) {
      ...
      // step.
      if (_isFixed(currentElement.element) || currentElement.fixed) {
        helperLayer.className += ' introjs-fixedTooltip';
        if(!document.querySelector('body').className.match(/\bintrojs-fixedElement\b/))
        	document.querySelector('body').className += ' introjs-fixedElement';
      } else {
        helperLayer.className = helperLayer.className.replace(' introjs-fixedTooltip', '');
        document.querySelector('body').className = document.querySelector('body').className.replace(' introjs-fixedElement', '');
      }
  ...
  /**
	...
	function bubbleText(step) {
		...
		if(step.title)
			introText += '<h1>'+step.title+'</h1>';

		if(step.subtitle)
			introText += '<h4>'+step.subtitle+'</h4>';

		if(step.intro)
			if(typeof step.intro === 'string')
				introText += '<p>'+step.intro+'</p>';
			else
				$.each(step.intro, function(index, line) {
					introText += '<p>'+line+'</p>';
				});

		if(step.loading)
			introText += '<div class="loading"></div>';
  ...
  function _showElement(targetElement) {
    ...
    if (oldHelperLayer != null) {
      ...
      self._lastShowElementTimer = setTimeout(function() {
        ...
        oldtooltipLayer.innerHTML = bubbleText(targetElement);
      ...
      tooltipTextLayer.innerHTML = bubbleText(targetElement);
      ...
      skipTooltipButton.onclick = function() {
        if (self._introItems.length - 1 == self._currentStep && typeof (self._introCompleteCallback) === 'function') {
          self._introCompleteCallback.call(self, self._targetElement, self._options.steps[self._currentStep], self._direction);
        }
    ...
    if (this._currentStep == 0 && this._introItems.length > 1) {
      ...
      if (typeof skipTooltipButton !== "undefined" && skipTooltipButton != null) {
        ...
        skipTooltipButton.parentNode.appendChild(skipTooltipButton);
      }
      if (typeof prevTooltipButton !== "undefined" && prevTooltipButton != null) {
      ...
      if(this._options.steps[this._currentStep].hideSkip)
      	if (typeof skipTooltipButton !== "undefined" && skipTooltipButton != null) {
      		skipTooltipButton.className = 'introjs-button introjs-skipbutton introjs-hidden';
      	}
    } else {
    ...
    if (typeof (this._introAfterChangeCallback) !== 'undefined') {
      this._introAfterChangeCallback.call(this, targetElement.element, this._options.steps[this._currentStep], this._direction, targetElement);
    }
  }
  ...
  function _scrollTo(scrollTo, targetElement, tooltipLayer) {
    ...
    if (!_elementInViewport(targetElement.element)) {
      ...
      if (top < 0 || targetElement.element.clientHeight > winHeight) {
        window.scrollBy({
        	left: 0, 
        	top: rect.top - ((winHeight / 2) -  (rect.height / 2)) - this._options.scrollPadding,
        	behavior: 'smooth'
        });
  ...
  introJs.fn = IntroJs.prototype = {
    ...
    addSteps: function(steps) {
      ...
      for(var index = 0; index < steps.length; index++) {
      	if(steps[index].access && !canUse(steps[index].access))
			break;
